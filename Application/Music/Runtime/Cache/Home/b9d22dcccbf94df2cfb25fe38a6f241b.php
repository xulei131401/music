...
<div class="page-content clearfix">
    <div id="Member_Ratings">
     ...
     <form  action="<?php echo U('Home/User/editHandle');?>" method="post" enctype="multipart/form-data">
    	 <input type="hidden" value = "<?php echo ($user['id']); ?>" name = "id"/>
         <table class="table table-striped table-bordered table-hover" id="sample-table">
		<thead>
		 <tr>
			<td width="30%">账号:</td>
			<td><input type="text" name = "username" placeholder="输入用户名" value = "<?php echo ($user['username']); ?>"></td>
		</tr>
		<tr>
			<td width="30%">密码:</td>
			<td><input type="text" name = "password" placeholder="输入密码" value = "<?php echo ($user['password']); ?>"></td>
		</tr>
		<tr>
			<td width="30%">手机:</td>
			<td><input type="text" name = "tel" placeholder="输入手机号" value = "<?php echo ($user['tel']); ?>"></td>
	    </tr>
	    <tr>
			<td width="30%">邮箱:</td>
			<td><input type="text" name = "email" placeholder="输入邮箱" value = "<?php echo ($user['email']); ?>"></td>
		</tr>
		<tr>
    		<td width="30%">地址:</td>
    		<td><input type="text" name = "address" placeholder="输入地址" value = "<?php echo ($user['address']); ?>"></td>
    	</tr>
    	<tr>
			<td width="30%">原始头像:</td>
			<td>
				<img src="/music/<?php echo ($user['pic']); ?>" width = "100" height = "100"><br/>
			</td>
		</tr>
		<tr>
			<td width="30%">新头像:</td>
		...
		<tr>
			...
			<td>
				...
				<option value="0">----请选择----</option>
					<?php if($user['vip'] == 1): ?><option value="1" selected="selected">普通用户</option>
						<option value="2">会员</option>
						<?php else: ?>
						<option value="1">普通用户</option>
						<option value="2" selected="selected">会员</option><?php endif; ?>
				</select>
           </td>
        </tr>
        <tr>
