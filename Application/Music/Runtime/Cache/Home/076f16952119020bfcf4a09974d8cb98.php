...
<meta http-equiv="Cache-Control" content="no-siteapp" />
        ...
        <!--[if lte IE 8]>
		  <link rel="stylesheet" href="<?php echo C('T_URL');?>/assets/css/ace-ie.min.css" />
		<![endif]-->
		<script src="<?php echo C('T_URL');?>/js/jquery-1.7.2.min.js"></script>
  ...
  <div class="administrator">
		...
		<thead>
		 <tr>
				...
				<th width="80px">管理组编号</th>
				<th width="50px">管理组名称</th>
	...
	<tbody>
	<?php if(is_array($group_show)): $k = 0; $__LIST__ = $group_show;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
      <td><label><input type="checkbox" class="ace"><span class="lbl"></span></label></td>
      <td><?php echo ($vo["id"]); ?></td>
      <td><?php echo ($vo["title"]); ?></td>
      <td>
  	<?php if(is_array($rules)): $k1 = 0; $__LIST__ = $rules;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k1 % 2 );++$k1; if($k == $k1): if(is_array($v)): $i = 0; $__LIST__ = $v;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v1): $mod = ($i % 2 );++$i; if($v1['title'] == null): ?><span></span>
  			<?php else: ?>
  			<i class="icon-circle"></i> 
			<span><?php echo ($v1["title"]); ?></span><?php endif; endforeach; endif; else: echo "" ;endif; endif; endforeach; endif; else: echo "" ;endif; ?>
      </td>
      <td class="td-status">
      <?php if($vo['status'] == 1): ?><span class="label label-success radius">已启用</span>
      	<?php else: ?>
      	<span class="label label-error radius">停用</span><?php endif; ?>
      </td>
      <td class="td-manage">
        <a href="<?php echo U('Home/Group/status',array('status'=>$vo['status'], 'id'=>$vo['id']));?>" title="停用"  class="btn btn-xs btn-success"><i class="fa fa-check  bigger-120"></i></a>  
        <a title="编辑"  target="_self" href="<?php echo U('Home/Group/edit',array('id'=>$vo['id']));?>"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>       
        <a title="删除" href="<?php echo U('Home/Group/delete',array('id'=>$vo['id']));?>" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
       </td>
