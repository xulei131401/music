...
<body>
	...
	<link rel="stylesheet" type="text/css" href="<?php echo C('T_URL');?>/css/Home/excellent.css">

	<div id="body">
		<div id="left">
			<h3>按时间</h3>
			<ul>
				<?php if($etime == 'd'): ?><li style="background-color:#7CDBFF;">全部</li>
				<?php else: ?>
					<li onclick="time('d')">全部</li><?php endif; ?>
				
				<?php if($etime == 'a'): ?><li style="background-color:#7CDBFF;">最近七天</li>
				<?php else: ?>
				<li onclick="time('a')">最近七天</li><?php endif; ?>

				<?php if($etime == 'b'): ?><li style="background-color:#7CDBFF;">最近一月</li>
				<?php else: ?>
				<li onclick="time('b')">最近一月</li><?php endif; ?>
				<?php if($etime == 'c'): ?><li style="background-color:#7CDBFF;">最近一季</li>
				<?php else: ?>
				<li onclick="time('c')">最近一季</li><?php endif; ?>
			</ul>
		</div>
		<div id="right">
			...
			<table id="r_title">
				<tr>
					...
					<td style="width:86px;text-align:right;">
						...
						<select name="order" onchange="efun(this.value)">
							<option value="1">按时间</option>
							<?php if($order == 2): ?><option value="2" selected>按浏览</option>
							<?php else: ?>
							<option value="2">按浏览</option><?php endif; ?>
						</select>
					</td>
				</tr>
			</table>
			...
			<?php if(is_array($excellentlist)): foreach($excellentlist as $key=>$v): ?><!-- 右边的精选区 需要循环的部分-->
			<table style="empty-cells:show;border-collapse:collapse;width:780px;margin-top:15px;border-bottom:1px solid #DCDCDC;table-layout:fixed;">
				<tr>
					<td rowspan="3"  style="width:104px;height:104px;"><a href="<?php echo U('Index/Excellent/songlist',array('id'=>$v['id']));?>" id="ename"><img id="logo" src="/music/<?php echo ($v["pic"]); ?>" width="104px" height="104px" ></a></td>
					<td rowspan="3" style="width:20px;"></td>
					<td style="height:25px;"><a href="/music/index.php/Index/Excellent/songlist/id/<?php echo ($v["id"]); ?>" id="ename"><?php echo ($v["name"]); ?></a></td>
					<td style="width:15%;font-size:14px;color:#333333;">制作人:<?php echo ($v["author"]); ?></td>
				</tr>
				<tr>
					<td colspan="2" style="height:52px;color:#999999;font-size:12px;line-height:22px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo ($v["desc"]); ?></td>
				</tr>
				<tr>
					<td colspan="2" style="text-indent:5px;height:26px;line-height:26px;">
						<a href="<?php echo U('Index/Audio/playExcellent',array('id'=>$v['id']));?>"><img src="<?php echo C('T_URL');?>/images/login/list.png" width="100px" height="26px" style="position:relative;top:5px;"></a>&nbsp;分享到:
						<a href="http://www.sina.com.cn/"><img src="<?php echo C('T_URL');?>/images/sina.png" style="border-radius:2px;margin-left:5px;"width="15px" height="15px"></a>
						<a href="http://t.qq.com/"><img src="<?php echo C('T_URL');?>/images/qqweibo.png" style="border-radius:2px;margin-left:5px;"width="15px" height="15px"></a>
						<a href="http://www.renren.com/"><img src="<?php echo C('T_URL');?>/images/renren.png" style="border-radius:2px;margin-left:5px;"width="15px" height="15px"></a>
						<a href="http://www.douban.com/"><img src="<?php echo C('T_URL');?>/images/douban.png" style="border-radius:2px;margin-left:5px;"width="15px" height="15px"></a>
					</td>
				</tr>
				<tr>
			...
			<!-- 右边的精选区 需要循环的部分--><?php endforeach; endif; ?>

		</div>

	</div>
	<div style="width:200px;text-align:center;margin:0 auto;"><?php echo ($page); ?></div>
<script type="text/javascript">
	order=document.getElementsByName("order")[0];
	function efun(a){
		window.location.href="/music/index.php/Index/Excellent/index/order/"+a+"/time/"+"<?php echo ($etime); ?>";
	}
	function time(b){
		window.location.href="/music/index.php/Index/Excellent/index/time/"+b+"/order/"+"<?php echo ($eorder); ?>";
	}
	function eplay(c){
		window.location.href="/music/index.php/Index/Excellent/eplay/eid/"+c;
	}
	$(function(){
		$("title").html("酷狗 乐库 精选");
	});
