   ...
   public function addHandle () {
      ...
      if (IS_POST) {
          ...
          $data['vip'] = I('vip');
            ...
            $upload->savePath  =     'Home/Uploads/user_pic/'; //设置管理员头像附件上传目录
    ...
    public function editHandle () {
      if (IS_POST) {
          ...
          //先处理用户上传头像
            ...
            $upload->subName   =    array('date','Ymd');
            $upload->rootPath   =   './Public/';       //网站跟目录是./
            $upload->savePath  =     'Home/Uploads/user_pic/'; //设置会员头像附件上传目录
            $rootPath =  $upload->rootPath;             //保存根路径 
            $info   =   $upload->upload();
            if(!$info) {        //当没有新头像上传时
    ...
    public function status(){
      if (IS_GET) {
        ...
        if($status){
          $data['status'] = 0;
          $user -> save($data);
          $this -> success("禁用成功!",U('Home/User/index'));
        }else{
          $data['status'] = 1;
          $user -> save($data);
          $this -> success("启用成功!",U('Home/User/index'));
        }
      } else {
        $this->error('非法请求!');
      }
